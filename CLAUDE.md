...
## 汇报风格（2026-09-19 用户要求）

**每轮回复要短。** 用户明确反馈"回复内容太多了"。

- **只报结论 + 需要用户拍板的点**。不复述过程、不重复已知信息、不解释刚刚已经说过的事
- 改动清单用表格或短列表，**不逐条展开理由**——细节在 commit 与 ADR 里，用户要看会自己看
- 验证证据**只贴命令与关键结果**，不贴大段日志
- 需要用户决策的，直接说「要你定 X」，**不要先铺垫三段背景**
- **不复述收场流程**；收场只在对话真的结束时做一次

> 例外：用户明确说「详细讲」「教我」时按需展开——那是在要解释，不是在要简报。
