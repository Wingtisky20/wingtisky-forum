...
**流程**：切分支 → 小步提交 → **开 PR（哪怕自己合并）** → 自查清单 → **Squash merge** → 删分支。

**为什么必须用 Squash merge**：feature 分支上允许存在过程性的 `wip:` 提交（探索本来就该随意），但合并进 main 时压平成一个规范提交。这样 **`git log --oneline main` 就是一条干净的迭代主线**——这正是"通过 commit 一步步看清迭代"的前提。细节见 §10.4。
